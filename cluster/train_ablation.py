...
# Ablation: remove outerness (col 11) and opposite_quality (col 12)
ABLATION_DROP = [11, 12]
IN_DIM = 13 - len(ABLATION_DROP)  # 11
...
def load_single_part_wrapper(args):
    ...
    try:
        ...
        feats = np.delete(feats, ABLATION_DROP, axis=1)
...
class FlatTriangleDataset(Dataset):
    def __init__(self, parts, augment_rot=True):
        ...
        if len(all_feats) > 0:
            ...
            self.X = np.zeros((0, IN_DIM), dtype=np.float32)
...
class ClampSupportNet(nn.Module):
    def __init__(self, in_dim=IN_DIM, hidden_dim=128, out_dim=2):
        super().__init__()
...
if __name__ == "__main__":
    ...
    model_save_path = "separation_model_11dim_ablation.pth"

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    num_cores = 16
    print(f"Running on {device} with {num_cores} CPU cores for loading.")
    print(f"Ablation: dropping features {ABLATION_DROP} (outerness, opposite_quality) -> {IN_DIM} dims")
    ...
    # 2. DATA SPLIT (70 / 15 / 15) — same seed as train.py for identical splits
    random.seed(42)
